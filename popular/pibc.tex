...
\begin{frame}
	...
	\begin{itemize}
		\item Elementary financial analysis
			\begin{itemize}
				\item Discounting
				\item NPV and IRR
				\item Cap rates
			\end{itemize}
		\item A standard ``monocentric'' model of location
			\begin{itemize}
				\item How price should change with location
				\item Which land uses go where
			\end{itemize}
		\item Supply elasticity, demand growth, and price growth
		\item Economists skepticism of affordable housing mandates
			\begin{itemize}
				\item Doesn't mean they are bad
				\item Do need a good reason for ``in kind'' vs cash
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item \$1 now generally deemed more valuable than \$1 in the future
			\begin{itemize}
				\item Inflation
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Note NPV and IRR fall if project takes 2 years vs 1:
			NPV: -2M + $\frac{2.5M}{1.1^2} \approx$ \$66K\\
		\item NPV higher if discount rate is lower, say 5\%
			NPV: -2M + $\frac{2.5M}{1.05} \approx$ \$380K
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{``More Money, More Sooner, More Better''}
	\begin{itemize}
		\item What makes NPV more positive?
			\begin{itemize}
				\item More money in, less money out
				\item Money comes in sooner
					\begin{itemize}
						\item If discount rate positive
						\item Then $\frac{1}{\left[1+d\right]^t}$ is decreasing in $t$
						\item World's simplest project completes in 1 year
						$$\text{NPV} = -2M + \frac{2.5M}{1.1} \approx 272K$$
						\item If project completes in 5 years
						$$\text{NPV} = -2M + \frac{2.5M}{1.1^5} \approx -\$448K$$
					\end{itemize}
				\item Lower discount rate
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Where should cap rates be higher?}
	\begin{itemize}
		\item Investors presumably get similar total returns
			\begin{itemize}
				\item In all locations
				\item In all asset classes
				\item Otherwise too many net sellers or buyers
			\end{itemize}
		\item If I buy, hold one period, then sell, ''unleveraged'' profit = 
			$$- \text{Price}_{0} + \text{Price}_{1} * \frac{cap rate}{1+d} + \frac{\text{Price}_{2}}{\left[1+d\right]^2}$$
		\item So if return is equal, $\uparrow$ cap rate $\Rightarrow$ $\downarrow \text{Price}_{2}$
		\item So higher cap rate in:
			\begin{itemize}
				\item Vancouver (San Francisco) apartment 2021?
				\item Toronto apartment 2025?
				\item Winnipeg (Des Moines) shopping center?
			\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Recap of today's learning objectives}
\begin{itemize}
	\item Elementary financial analysis
		\begin{itemize}
			\item Discounting
